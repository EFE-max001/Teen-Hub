    ...
    <div className={`mb-10 md:mb-14 ${centered ? 'text-center' : ''}`}>
      {eyebrow && (
        <div className={`inline-flex items-center gap-3 mb-3 ${centered ? 'justify-center' : ''}`}>
          ...
          <span className="font-orbitron text-[10px] text-cyan-400 tracking-[0.4em] uppercase">
            {eyebrow}
          </span>
          <div className="w-6 h-px bg-cyan-500/50" />
        </div>
      ...
      <div className={`w-14 h-0.5 bg-gradient-to-r from-cyan-600 to-cyan-400 mt-3 mb-4 ${centered ? 'mx-auto' : ''}`} />
      {subtitle && (
