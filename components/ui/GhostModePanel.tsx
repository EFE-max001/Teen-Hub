...
export default function GhostModePanel() {
  ...
  return (
    <div className="relative">
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className={`flex items-center gap-1.5 px-3 py-1.5 border font-orbitron text-[9px] tracking-widest transition-colors ${
          isActive
          ...
          <div className="absolute right-0 top-full mt-1 z-50 w-80 bg-[#0a0a0f] border border-violet-500/30 shadow-2xl shadow-violet-900/20">
            <div className="px-4 py-3 border-b border-violet-500/20">
              <div className="flex items-center justify-between">
                <p className="font-orbitron text-[10px] text-violet-400 tracking-widest">GHOST IDENTITY</p>
                {ghost && !setupMode && (
                  <button
                    onClick={() => { setSetupMode(true); setNameInput(ghost.ghostName); setRankInput(ghost.ghostRank) }}
                    className="font-orbitron text-[9px] text-slate-600 hover:text-slate-300 transition-colors"
                  >EDIT</button>
                )}
              </div>
              <p className="font-rajdhani text-slate-500 text-xs mt-0.5">
            ...
            <div className="p-4">
              {loading ? (
                ...
                <div className="space-y-3">
                  <div>
                    <label className="font-orbitron text-[9px] text-slate-500 tracking-widest block mb-1">GHOST NAME</label>
                    <input
                      autoFocus
                      value={nameInput}
                      ...
                      maxLength={24}
                      className="w-full bg-black/60 border border-violet-500/30 text-white font-rajdhani text-sm px-3 py-2 focus:outline-none focus:border-violet-400/60 transition-colors"
                    />
                  </div>
                  <div>
                    <label className="font-orbitron text-[9px] text-slate-500 tracking-widest block mb-1">GHOST RANK</label>
                    <select
                      value={rankInput}
                      onChange={e => setRankInput(e.target.value)}
                      className="w-full bg-black/60 border border-violet-500/30 text-white font-orbitron text-xs px-3 py-2 focus:outline-none focus:border-violet-400/60"
                    >
                      {RANKS.map(r => (
                        <option key={r} value={r}>{r}</option>
                      ))}
                    </select>
                    <p className="font-rajdhani text-slate-700 text-[10px] mt-1">SSS rank is excluded — it would give you away.</p>
                  </div>
                  {setupError && <p className="font-rajdhani text-red-400 text-xs">{setupError}</p>}
                  <div className="flex gap-2">
                    <button
                      onClick={saveGhost}
                      disabled={setupSaving || !nameInput.trim()}
                      className="flex-1 font-orbitron text-[10px] tracking-widest text-violet-300 border border-violet-500/50 py-2 hover:bg-violet-900/20 transition-colors disabled:opacity-40"
                    >{setupSaving ? '…' : ghost ? 'UPDATE' : 'CREATE GHOST'}</button>
                    {ghost && (
                      <button
                        onClick={() => { setSetupMode(false); setSetupError('') }}
                        className="font-orbitron text-[10px] tracking-widest text-slate-500 border border-slate-700 px-3 py-2 hover:bg-slate-800/40 transition-colors"
                      >✕</button>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 p-3 border border-violet-500/20 bg-violet-900/10">
                    ...
                    <div>
                      <p className="font-orbitron text-sm text-white">{ghost.ghostName}</p>
                      <p className="font-orbitron text-[9px] text-violet-400">Rank {ghost.ghostRank}</p>
                    </div>
                    <div className={`ml-auto w-2 h-2 rounded-full ${isActive ? 'bg-violet-400 animate-pulse' : 'bg-slate-700'}`} />
                  </div>

                  <div className="space-y-2">
                    <p className="font-rajdhani text-slate-400 text-xs leading-relaxed">
                      {isActive
                    ...
                    <button
                      onClick={toggle}
                      disabled={toggling}
                      className={`w-full font-orbitron text-[10px] tracking-widest py-2.5 border transition-colors disabled:opacity-40 ${
                        isActive
                  ...
                  <div className="border-t border-violet-500/10 pt-3">
                    <p className="font-orbitron text-[9px] text-slate-700 tracking-widest uppercase mb-2">You can send messages as</p>
                    <div className="flex gap-2">
                      <div className={`flex-1 text-center py-2 border font-orbitron text-[9px] tracking-widest cursor-pointer transition-colors ${
