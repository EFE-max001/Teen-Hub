...
const STATUS_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  ...
  DRAFT:       { label: 'Draft',       color: 'border-slate-500/40 text-slate-400 bg-slate-900/20',    dot: 'bg-slate-400' },
  PAID:        { label: 'Paid',        color: 'border-portal-emerald/40 text-portal-emerald bg-portal-emerald/10', dot: 'bg-portal-emerald' },
  FAILED:      { label: 'Failed',      color: 'border-red-500/40 text-red-400 bg-red-900/20',          dot: 'bg-red-400' },
  EXPIRED:     { label: 'Expired',     color: 'border-slate-500/40 text-slate-500 bg-slate-900/20',    dot: 'bg-slate-500' },
  CANCELLED:   { label: 'Cancelled',   color: 'border-slate-500/40 text-slate-500 bg-slate-900/20',    dot: 'bg-slate-500' },
  REFUNDED:    { label: 'Refunded',    color: 'border-cyan-500/40 text-cyan-400 bg-cyan-900/20',       dot: 'bg-cyan-400' },
}
