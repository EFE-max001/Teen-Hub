...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // Same class of bug as pages/api/arena/[id]/start.ts previously: an
    // uncaught exception here lets Next's dev server render its HTML crash
    // overlay instead of JSON, which shows up client-side as "Unexpected
    // token '<' ... is not valid JSON" with zero diagnostic value. Given
    // how many new Prisma models this feature added (PaymentRequest,
    // PaymentAuditLog, etc.), an out-of-sync Prisma client is the most
    // likely cause — this message says so directly if that's it.
    console.error('[founder/payments] failed:', err)
